...
router.post(
  ...
  function (req, res, next) {
    ...
    global.db.all(ticketsQuery, [eventId], function (err, ticketTypes) {
        ...
        if (quantity > ticket.available) {
          return res.redirect(
            backUrl +
              '?error=' +
              encodeURIComponent(
                'Only ' + ticket.available + ' "' + ticket.name + '" ticket(s) are available'
              )
          );
        }
        if (quantity > 0) {
      ...
      if (totalRequested === 0) {
        return res.redirect(
          backUrl + '?error=' + encodeURIComponent('Please select at least one ticket to book')
        );
      }
      ...
      global.db.serialize(function () {
        ...
        const insertBooking = 'INSERT INTO bookings (event_id, attendee_name) VALUES (?, ?)';
        global.db.run(insertBooking, [eventId, req.body.attendee_name.trim()], function (err) {
          if (err) {
            ...
            global.db.run('COMMIT', function (err) {
              ...
              res.redirect(
                backUrl +
                  '?msg=' +
                  encodeURIComponent(
                    'Booking confirmed for ' + req.body.attendee_name.trim() + '. Enjoy the event!'
                  )
              );
            });
          });
