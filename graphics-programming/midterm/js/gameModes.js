function setupMode1() {
  ...
  const coloredPositions = [
    [
      table.x + table.tableWidth / 2 - 50,
      table.y - table.tableLength / 4,
      [255, 255, 0]
    ], // Yellow
    [
      table.x + table.tableWidth / 2 - 25,
      table.y - table.tableLength / 4,
      [0, 0, 255]
    ], // Blue
    [
      table.x + table.tableWidth / 2,
      table.y - table.tableLength / 4,
      [128, 0, 128]
    ], // Pink
    [
...
function setupMode2() {
  ...
  for (let c = 0; c < numClusters; c++) {
    const clusterX = random(
      table.x - table.tableWidth / 3,
      table.x + table.tableWidth / 3
    ) // Use tableWidth for horizontal
    const clusterY = random(
      table.y - table.tableLength / 3,
      table.y + table.tableLength / 3
    ) // Use tableLength for vertical
